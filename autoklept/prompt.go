...
- When you output, do not write anything before or after the raw output you formatted from the original content. Omit any backticks markdown open / close blocks.`
	...
	StdPrompt = "Extract out all actual user content from the following HTML. "
)
...
func buildPromptString(input PromptInputTag, output PromptOutputTag, htmlContent string) string {
	var sb strings.Builder
	_, _ = sb.WriteString(StdPrompt)
	sb.WriteString("\n")
	_, _ = sb.WriteString(GetInputTagText(input))
	sb.WriteString("\n")
	_, _ = sb.WriteString(GetOutputTagText(output))
	sb.WriteString("\n")
	_, _ = sb.WriteString(htmlContent)
	return sb.String()
}
